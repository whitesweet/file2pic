...
def read_all_file_files_in_directory(directory_path):
    # 获取文件夹下所有的 .file 文件路径
    file_files = glob.glob(os.path.join(directory_path, "*.txt"))
...
def generatePic(file_name, binary_data):
...
# 调用函数读取所有 .file 文件内容
all_file_contents = read_all_file_files_in_directory(directory_path)

for file_name, content in all_file_contents:
    generatePic(file_name, content)
